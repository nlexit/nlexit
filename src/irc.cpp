...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #nlexitTEST2\r");
            Send(hSocket, "WHO #nlexitTEST2\r");
        } else {
            // randomly join #nlexit00-#nlexit05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #nlexit%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #nlexit%02d\r", channel_number).c_str());
        }
